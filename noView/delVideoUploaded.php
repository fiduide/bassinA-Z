...
if(!empty($_GET['id'])){
    ...
    $suppV = $bdd->query('SELECT access FROM videouploaded WHERE id = "'.htmlspecialchars($_GET['id']).'"');

    $supp = $suppV->fetch();

    RepEfface('../uploads/videos/'.$supp['access']);

    $req = $bdd->prepare('DELETE FROM videouploaded WHERE id = ?');
